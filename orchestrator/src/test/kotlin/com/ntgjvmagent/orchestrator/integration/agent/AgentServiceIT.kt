...
class AgentServiceIT(
    ...
    @Autowired private val repo: AgentRepository,
) : BaseIntegrationTest() {
    ...
    fun `should return only active agents`() {
        repo.save(Agent(name = "ActiveA", model = "gpt-4o-mini", active = true))
        repo.save(Agent(name = "InactiveB", model = "gpt-4o-mini", active = false))
    ...
    fun `should soft delete agent`() {
        ...
        val deleted = repo.findById(saved.id!!).orElseThrow()
        assertNotNull(deleted.deletedAt)
        assertTrue(ZonedDateTime.now().isAfter(deleted.deletedAt))
    }
