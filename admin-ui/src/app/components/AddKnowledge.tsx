...
const formSchema = z.object({
    knowledge: z.custom<FileList>((value) => value instanceof FileList && value.length > 0, { error: "File is required" })
        .refine(f => checkFileType(f[0]) && (isPDFFile(f[0]) ? isValidPDFFile(f[0]) : true), { error: "File extension not supported" })
...
const AddKnowledge = () => {
    ...
    const { register, handleSubmit, formState: { errors }, setError, reset } = useForm<KnowledgeInput>({ resolver: zodResolver(formSchema) });
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className='flex gap-4'>
            ...
            <button type='submit' disabled={isPending} className='h-fit p-2 rounded border border-black px-4 cursor-pointer enabled:hover:bg-black enabled:hover:text-white disabled:cursor-not-allowed'>
